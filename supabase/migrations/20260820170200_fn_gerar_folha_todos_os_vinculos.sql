-- A folha gerencial passa a incluir terceiro e diarista, com gratificacao e
-- encargo individual.
--
-- O que muda:
--
--   1. O loop deixa de ser `vinculo = 'clt'` e passa a ser os tres vinculos.
--      A base do mes depende do vinculo:
--        clt / terceiro -> salario mensal do cadastro;
--        diarista       -> soma das diarias DA COMPETENCIA que ainda nao foram
--                          pagas, com a MESMA definicao de "em aberto" que a
--                          fn_fechar_diarias usa. Diarista que nao trabalhou no
--                          mes nao gera item.
--   2. Gratificacao salarial entra no bruto, no liquido e no custo, e NAO entra
--      na base dos encargos nem da provisao.
--   3. Encargo pode ser individual (colaboradores.encargos_percentual). E por
--      isso que terceiro e diarista podem entrar sem carregar encargo de CLT.
--   4. INSS e IRRF continuam SO para CLT. Retencao de terceiro e de diarista e
--      regra fiscal que o Tiago ainda vai declarar, e regra fiscal nao se
--      inventa. A base do desconto do CLT passa a incluir a gratificacao:
--      gratificacao habitual integra a remuneracao do trabalhador. O que ela
--      nao afeta e o encargo PATRONAL.
--   5. Regerar PRESERVA as linhas que o Tiago editou a mao.
--
-- O que NAO muda: as travas da cascata de adiantamento, palavra por palavra.
-- Elas custaram medicao de dinheiro invisivel para existir; abaixo estao
-- inteiras, com os comentarios originais.
...
$doc$Gera (ou regera, em rascunho) a folha gerencial da competencia.

Quem entra: colaborador ativo de vinculo clt, terceiro ou diarista.
Base do mes: salario do cadastro (clt/terceiro) ou soma das diarias em aberto
da competencia (diarista, mesma definicao de "em aberto" da fn_fechar_diarias).
Item que ficaria com base 0, gratificacao 0 e nenhuma hora nao e criado.

Gratificacao (colaboradores.gratificacao ou o valor editado a mao no item) soma
no bruto, no liquido e no custo total, e NAO entra na base de encargo nem de
provisao. Regra do Tiago.

Encargo: percentual individual do colaborador quando existir (uma linha
"Encargos" sem grupo de recolhimento, logo sem guia), senao os folha_encargos
ativos discriminados (com grupo, gera guia).

INSS/IRRF: so vinculo clt, base = salario base + gratificacao. Retencao de
terceiro e diarista e regra fiscal ainda nao declarada e nao e inventada aqui.

Regerar PRESERVA salario base, gratificacao e percentual de encargo dos itens
marcados editado_manualmente (snapshot por colaborador antes do delete).

p_encargos_pct e LEGADO e ignorado.$doc$;
