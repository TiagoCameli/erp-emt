-- Semeia o recurso rh.encargos (registrado em config/recursos.ts nesta mesma task)
-- espelhando EXATAMENTE quem ja tem rh.folha — o cadastro de encargos e operado por
-- quem opera a folha gerencial. Lido via MCP (2026-07-27): rh.folha em perfil_permissoes
-- = 7 linhas (ver 3 / criar 2 / editar 2); usuario_permissoes = 3 linhas.
...
--
-- Padrao seguido: 20260725110002_perm_jornadas.sql.
--
-- Rollback:
--   delete from public.usuario_permissoes where recurso = 'rh.encargos';
--   delete from public.perfil_permissoes where recurso = 'rh.encargos';

-- 1) Matriz de perfil: mesmos perfis/acoes que rh.folha.
insert into public.perfil_permissoes (perfil_id, recurso, acao)
select pp.perfil_id, 'rh.encargos', pp.acao
