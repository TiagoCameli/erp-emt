-- =============================================================
-- Importacao de lancamentos: tres correcoes achadas na conferencia
--
-- 1. NUMERO DO DOCUMENTO NAO SOBRESCREVE O NUMERO DO LANCAMENTO.
--    lancamentos.numero e o numero do ERP (LAN-2026-0001), gerado por
--    trg_lancamentos_numero. A coluna "Numero do documento" da planilha
--    e outra coisa: a nota, o boleto, o recibo do fornecedor. A versao
--    anterior gravava um no lugar do outro, e como 2.412 das 7.253
--    linhas trazem documento, a carga apagaria a numeracao do ERP em
--    um terco dos lancamentos (e nada garante que o numero do
--    fornecedor seja unico). O dado nao se perde: vai para as
--    observacoes, no mesmo formato ja usado para OC e plano de contas.
--
-- 2. PARCELA PAGA GANHA data_programada_origem. Estava setando
--    data_programada sem dizer de onde ela veio, deixando a coluna
--    nula com a data preenchida. Veio do vencimento.
--
-- 3. FASE 1 VALIDA A DATA DO LANCAMENTO. fn_salvar_lancamento exige
--    data_compra e levanta excecao sem ela. Sem essa checagem, uma
--    linha sem data derruba a carga NO MEIO da fase 2, com parte
--    gravada, em vez de aparecer na lista de erros junto com as
--    outras. A fase 1 existe justamente para isso: erro de planilha
--    se mostra todo de uma vez, antes de gravar qualquer coisa.
-- =============================================================
...
begin
  ...
  for v_linha in select * from jsonb_array_elements(p_linhas) loop
    ...
    -- Data do lancamento: fn_salvar_lancamento exige, entao valida aqui.
  ...
  for v_linha in select * from jsonb_array_elements(p_linhas) loop
    ...
    -- O numero do documento entra aqui, e nao em lancamentos.numero,
    -- que e a numeracao do ERP.
...
-- Carga em lote precisa de mais tempo que consulta de tela. O CREATE OR
-- REPLACE acima nao preserva o SET da funcao, entao reaplica.
...
comment on function public.fn_importar_lancamentos(jsonb) is
  'Importa lancamentos de planilha. Valida tudo antes de gravar nada, reusa fn_salvar_lancamento por linha e marca parcela como paga quando a linha traz data de pagamento (carga de historico). O numero do documento da planilha vai para observacoes: lancamentos.numero e a numeracao do ERP.';
