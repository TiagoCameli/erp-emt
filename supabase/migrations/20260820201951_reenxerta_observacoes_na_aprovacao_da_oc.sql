-- Reenxerta a copia de observacoes em fn_aprovar_ordem_compra.
--
-- Terceira rodada de sobrescrita da MESMA funcao em uma tarde. A sequencia:
--   194612 aprovar_oc_cria_bloco_de_forma            (outra frente: bloco)
--   195422 oc_observacoes_descem_para_o_lancamento   (eu: apaguei o bloco)
--   200020 aprovar_oc_bloco_de_forma_e_observacoes_juntos (eu: mesclei os dois)
--   200546 aprovar_oc_desce_a_divisao_por_forma      (outra frente: apagou obs)
--   ESTA   reenxerta_observacoes_na_aprovacao_da_oc  (eu: mesclo de novo)
--
-- CREATE OR REPLACE substitui o corpo INTEIRO: quem escreve a partir de uma
-- leitura antiga apaga o trabalho do outro sem conflito, sem erro e sem aviso.
-- Nao existe merge de tres vias em Postgres.
--
-- Base desta versao: a definicao viva lida imediatamente antes deste apply
-- (md5 372f99a20e4c1d2196b7ecd96b505636), que e a da 200546 com a divisao por
-- forma. Desta base saem APENAS tres acrescimos, todos de observacoes:
--   1. declare v_observacoes text
--   2. observacoes no select / v_observacoes no into
--   3. observacoes na lista de colunas do insert / o nullif(btrim(...)) no values
-- Nada mais foi tocado: os portoes de forma, os blocos e o join por
-- forma_pagamento_id sao os deles, letra por letra.
--
...
begin
  ...
  -- duas.
  --
  -- As observacoes descem junto, e o motivo e o mesmo do numero do documento:
  -- elas carregam chave PIX, CNPJ, data combinada de pagamento e avisos que
  -- Compras escreveu PARA quem paga, e sem a copia o Financeiro nunca as le.
  -- Normalizadas porque a tela testa `observacoes ? ...` e string de branco e
  -- TRUTHY -- desenharia a secao "Observacoes" vazia. O conjunto de caracteres
  -- vai EXPLICITO: btrim(x) sozinho corta so espaco, entao E'  \n \t '
  -- sobreviveria como E'\n \t'.
...
-- Backfill de novo: OCs aprovadas na janela em que a copia estava fora do ar
-- nasceram sem observacao. Idempotente, so preenche o que esta vazio.
