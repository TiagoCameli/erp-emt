-- Reclassificação de centro de custo de 131 lançamentos, vinda da planilha
-- lancamentos-2026-08-14.xlsx que o Tiago exportou da tela de Lançamentos,
-- editou e devolveu em 14/08/2026.
--
-- O QUE A PLANILHA MUDOU, e só isto: 131 lançamentos saíram de 'Escritório
-- Central' para a obra ou o centro que realmente consumiu o custo.
--
--     54 -> 007 - AC 405 - Lote 2
--     35 -> 003 - Recuperação do Ramal do Gama
--     24 -> 009 - Manutenção da Rodovia BR-364/AC - Lote 09 & 10
--     18 -> Manutenção/Documentação de Equipamentos
--
-- Como isso foi provado, e não suposto: as 21 colunas da exportação foram
-- comparadas contra o banco como multiset (um hash por coluna, independente da
-- ordem das linhas -- que a planilha perdeu, porque foi reordenada por Data da
-- compra no Excel). 19 colunas batem byte a byte. 'Rateio' bate depois de
-- normalizar a ordem dos nomes dentro da célula. Só 'Centro de custo' divergiu.
-- Depois, linha a linha: as 5.848 linhas casam 1:1 por assinatura, ZERO órfãs
-- dos dois lados. Nenhum valor, data, status, parcela, fornecedor, categoria,
-- forma de pagamento, conta bancária ou observação mudou.
--
-- POR QUE ASSINATURA E NÃO O NÚMERO DO LANÇAMENTO: 'Número' não é único nesta
-- base. A carga do Mais Controle repetiu o mesmo LAN-2026-xxxx em até 10
-- lançamentos diferentes (5.848 linhas, apenas 587 números distintos), e a
-- planilha não exporta o id. A linha é então identificada por um md5 das 19
-- colunas que não mudaram; cada assinatura abaixo casa com exatamente 1
-- lançamento, e o bloco aborta se isso deixar de valer.
--
-- ESCREVE SÓ em lancamento_rateios.centro_custo_id. `lancamentos.centro_custo_id`
-- é coluna legada e está NULL nas 5.848 linhas: preencher ela criaria uma segunda
-- fonte de verdade de centro de custo. O valor do rateio não é tocado (são todos
-- de rateio único, com valor igual ao do lançamento).
--
-- A auditoria sai de graça: trg_audit_lancamento_rateios grava antes/depois de
-- cada linha em audit_log.

do $$
declare
  -- Cada alvo são 11 caracteres: 10 de assinatura + 1 de índice do centro novo.
  v_enc text :=
  ...
  v_ambiguas int;
begin
  create temp table _alvo (sig text primary key, cc_novo uuid not null) on commit drop;

  insert into _alvo (sig, cc_novo)
  ...
  -- A MESMA expressão de assinatura que gerou o diff contra a planilha: as 19
  -- colunas da exportação menos 'Centro de custo' e 'Rateio'. Mudar qualquer
  -- coisa aqui invalida as assinaturas acima.
  ...
  -- Assinatura casando com mais de um lançamento tornaria a identificação da
  -- linha um chute. Aborta antes de escrever, em vez de reclassificar o errado.
  select count(*) into v_ambiguas
  from (
    select s.sig from _sig s join _alvo a on a.sig = s.sig
    group by s.sig having count(*) <> 1
  ) x;
  if v_ambiguas <> 0 then
    raise exception '% assinatura(s) casando com mais de um lançamento', v_ambiguas;
  end if;

  update lancamento_rateios r
     set centro_custo_id = a.cc_novo
    from _sig s
    join _alvo a on a.sig = s.sig
   where r.lancamento_id = s.id
     -- As duas condições que a planilha mostra: sai de Escritório Central e é
     -- rateio único. Se o banco já mudou desde a exportação, não atualiza.
     and r.centro_custo_id = '0a327d7e-6e2d-40d9-a87b-cf9b4a76be2e'::uuid
     and (select count(*) from lancamento_rateios r2
          where r2.lancamento_id = s.id) = 1;

  get diagnostics v_afetadas = row_count;
  if v_afetadas <> 131 then
    raise exception 'esperava atualizar 131 rateios, atualizou %', v_afetadas;
  end if;

  raise notice 'reclassificados % rateios de centro de custo', v_afetadas;
end $$;
