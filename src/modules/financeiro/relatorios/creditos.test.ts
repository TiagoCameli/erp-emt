...
describe("montarCreditos", () => {
  it("ordena por saldo devedor, do maior para o menor", () => {
    const { contratos } = montarCreditos(
      [
  ...
  it("soma os totais por centavos, e não somando os reais", () => {
     ...
     */
    const { totalSaldo, totalContratado, totalPago } = montarCreditos(
      [
  ...
  it("rotula o mês como MM/YYYY e soma o período", () => {
    ...
    const { proximosMeses, totalProximosMeses } = montarCreditos([], meses);
  ...
  it("devolve tudo zerado quando nada foi marcado como crédito", () => {
    const vazio = montarCreditos([], []);
