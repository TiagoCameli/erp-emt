...
describe("gravarImportacao", () => {
  ...
  it("aditivo com item escolhido grava o item_id da versão anterior", async () => {
    ...
    await expect(gravarImportacao(V, MAPA, { ...OK, itemPorOrdem: { 2: "b" } })).resolves.toEqual({ ok: true, linhas: 2 });
  ...
  it("mapeamento inválido não chega ao arquivo", async () => {
    await expect(gravarImportacao(V, { ...MAPA, linhaCabecalho: 0 }, OK)).resolves.toEqual({ erro: "Mapeamento de colunas inválido" });
    expect(estado.chamadas).toEqual([]);
  });

  it("aba que não existe vira erro de tela, não exceção", async () => {
    await expect(gravarImportacao(V, { ...MAPA, aba: "Outra" }, OK)).resolves.toEqual({ erro: "A aba Outra não existe no arquivo" });
  });
});
