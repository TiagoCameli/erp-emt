...
export interface CondicaoLista {
  ...
  ativo: boolean;
  qtdParcelas: number;
  /** Resumo legível das parcelas, ex.: "À vista" ou "0/30/60 dias". */
...
/** Monta o resumo legível a partir das parcelas (já ordenadas por dias). */
function resumoDeParcelas(parcelas: CondicaoParcela[]): string {
  ...
  return `${parcelas.map((p) => p.diasOffset).join("/")} dias`;
}
