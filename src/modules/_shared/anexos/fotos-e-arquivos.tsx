...
import {
  ...
  TIPOS_ARQUIVO,
  TIPOS_FOTO,
} from "@/modules/_shared/anexos/fotos-e-arquivos-regras";

/**
 * Fotos e arquivos de um registro (saída, entrada ou transferência de combustível,
 * OS da manutenção): o AnexosUploader da origem sobre os anexos canônicos do ERP.
 *
 * Na criação o registro ainda não existe, então os arquivos esperam numa fila
 * (`FilaFotosEArquivos`) e sobem logo depois do salvar (`subirFilaFotosEArquivos`),
 * pendurados no id que o banco devolveu. Na edição e no detalhe o registro existe
 * e cada arquivo sobe na hora (`FotosEArquivos`).
 */
...
export interface FilaDeFotosEArquivos {
  fotos: File[];
  arquivos: File[];
}

export const FILA_VAZIA: FilaDeFotosEArquivos = { fotos: [], arquivos: [] };

export function filaTemAlgo(fila: FilaDeFotosEArquivos): boolean {
 ...
 */
export function subirFilaFotosEArquivos(
  entidade: EntidadeAnexo,
  id: string | null,
  fila: FilaDeFotosEArquivos,
...
export interface FilaFotosEArquivosProps {
  fila: FilaDeFotosEArquivos;
  onMudar: (fila: FilaDeFotosEArquivos) => void;
  ocupado?: boolean;
...
export function FilaFotosEArquivos({ fila, onMudar, ocupado = false, soFotos = false }: FilaFotosEArquivosProps) {
  function mudar(grupo: keyof FilaDeFotosEArquivos, proxima: File[]) {
 ...
 */
export function useAnexosDoRegistro(entidade: EntidadeAnexo, id: string | null) {
...
export interface FotosEArquivosProps {
  entidade: EntidadeAnexo;
  entidadeId: string;
...
export function FotosEArquivos({ entidade, entidadeId, anexos, erro, podeEditar, onMudou }: FotosEArquivosProps) {
  if (erro) {
    return (
