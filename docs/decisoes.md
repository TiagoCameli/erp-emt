...
**O mesmo bug achado duas vezes, por dois caminhos.** Testando o painel filtrado por equipamento, o
cartão "Custo por centro de custo" veio como traço: `fn_rel_custo_centro_custo` recebe
`p_centros uuid[]` e o painel mandava `p_centro_custo` singular. Corrigi no meu branch — e o merge
com a main mostrou que **outra frente já tinha corrigido em 28/08**, com diagnóstico melhor: o nome
antigo só ia junto quando havia filtro escolhido, porque o supabase-js OMITE chave `undefined` do
corpo, então sem filtro o PostgREST resolvia pelos defaults e ninguém via nada. Ficou o comentário
deles. Vale como lembrete de que `supabase.rpc` aceita chave que não existe na assinatura sem o
`tsc` reclamar, e de que dois branches longos acham a mesma coisa em paralelo.
...
## 25/09/2026 — Aplicações financeiras: posição do extrato gera o rendimento, e a abertura fica fora do DRE

**Pedido do Tiago:** controlar as aplicações com o saldo batendo com o banco, mostrando rendimento e rentabilidade. Aplicar e resgatar **continuam** transferência com etapa (PR #313), e nada aqui mexe nelas.

**A regra, por aplicação.** Uma vez por mês (ou quando ele quiser) grava-se a **posição do extrato**: o saldo LÍQUIDO numa data, com o PDF. Rendimento da posição = líquido desta − líquido da anterior − aplicado + resgatado em (anterior, esta]. A posição é saldo de fim de dia, então a transferência do mesmo dia entra no período dela. Cada posição gera **um** lançamento (`origem = 'aplicacao'`, `origem_id` = a posição), já pago na subconta, rateio 100% na etapa. Por isso o saldo da subconta = soma das posições + o movimento depois da última. Um índice único em `lancamentos (origem_id) where origem = 'aplicacao'` torna duplicidade impossível, não só improvável. Regravar atualiza o mesmo lançamento. Excluir (soft delete com motivo) estorna, e a posição seguinte é recalculada porque ganhou outra anterior. Transferência de aplicação lançada, editada ou apagada com data antiga recalcula por trigger.

**Categorias.** Rendimento ≥ 0 vai em "Juros de aplicações financeiras" (receita, financeira): entra no DRE e no fluxo de caixa realizado. Rendimento < 0 vai em categoria **separada** (decisão do Tiago), "Rendimento negativo de aplicações financeiras" (despesa, financeira), como lançamento a pagar, porque `lancamentos.valor >= 0`.

**A abertura (mecanismo aprovado pelo Tiago).** A posição de 25/09/2026 (CDB 5.016.513,57, Fundo 1.000.971,18) carrega saldo anterior a abril e rendimento passado: 90.374,18 + 13.923,78 = 104.297,96. Ela é a primeira posição de cada aplicação, marcada `e_abertura`, e o lançamento dela vai em "Ajuste de abertura de aplicações", de natureza **movimentação**: fica fora do fluxo e do custo por construção, e o Tiago pediu que nem apareça no DRE (`fn_rel_dre` tira origem `aplicacao` + movimentação). Para ele mover a subconta, `fn_rel_posicao_bancaria` e `fn_extrato_conta` ganharam UMA exceção estreita: movimentação conta no saldo quando `origem = 'aplicacao'`. Recusadas: **natureza nova** (19 funções e 10+ arquivos TS; natureza desconhecida cai em operacional no `calculo.ts`, então um deploy fora de ordem viraria 104 mil de receita operacional) e **ajuste no `saldo_inicial`** (um número por conta, sem separar CDB e Fundo, e quebraria a identidade saldo_inicial = aplicado − resgatado até 26/08 do #313). A abertura não se exclui, só se regrava. Posição anterior a ela é recusada.

**Travas.** `fn_estornar_pagamento`, `fn_definir_rateio_lancamento` e `fn_definir_parcelas_lancamento` recusam lançamento de origem `aplicacao` ("regrave ou exclua a posição"). `fn_salvar_lancamento` e `fn_excluir_lancamento` já recusavam. Posição não aceita data futura. Competência fechada bloqueia pela `fn_exigir_competencia_aberta`.

**Permissão.** Recurso novo `financeiro.aplicacoes` (ver, editar), para os Admins. A posição é saldo: a RLS de `aplicacao_posicoes` e a `fn_aba_aplicacoes` exigem também `fn_pode_ver_saldo` da subconta (que herda o da conta-mãe). Sem permissão a linha **não vem**, e ausência não é zero. Pendência aceita: o PDF do extrato anexado segue a regra do anexo (`financeiro.aplicacoes/ver`), sem o filtro de saldo; hoje só Admin tem a aba.

**CDI (escolha deixada para mim).** Guardo a **série 12** (diária, 6 casas) e a 4391 (mensal, 2 casas). A 12 dá o % do CDI exato entre duas posições que não caem no fim do mês; a 4391 com 2 casas erraria até 0,4% (1,09 contra 1,0931). Conferido: a 12 composta bate com a 4391 de jan a set/2026. A 4391 é conferência e reserva: se a diária não cobrir, a aba usa a mensal quando o período é o mês cheio, e há carga manual. Rota `/api/cdi`, cron diário às 13:00 UTC, com `CRON_SECRET` e na lista de rotas públicas do middleware (sem isso o cron ia para o /login, como já tinha acontecido com a faxina).

**Rentabilidade.** Dietz modificado por posição (o aporte pesa pelos dias que ficou aplicado no período), composto no mês. Sem isso, aplicar 2 milhões no dia 21 derruba o % do mês. Mês sem posição sai com rendimento NULO.
