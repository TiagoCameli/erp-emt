...
3. **Valor é NUMERIC(14,2); taxa digitada aceita 4 casas.** Float é proibido para valores. **VALOR** é dinheiro que alguém paga ou recebe (`valor_total`, parcela, rateio, pagamento, salário, diária): duas casas, porque centavo é a unidade em que o boleto é pago e em que o extrato OFX concilia. **TAXA** é o número que multiplica para virar valor (preço unitário, quantidade, percentual, extensão): NUMERIC(_,4), porque diesel é vendido a R$ 6,3947 o litro e arredondar a taxa erra o valor final por item. As duas constantes ficam em `src/lib/casas-decimais.ts` e valem para schema, input e coluna ao mesmo tempo — campo novo lê de lá, nunca escreve o número solto. Exibição BRL: R$ 1.234,56, alinhado à direita, `tabular-nums`. **Exceção (22/09/2026): Frete, Combustível e Manutenção** guardam o VALOR com 4 casas, NUMERIC(14,4), pela constante própria `CASAS_VALOR_OPERACIONAL` (conta corrente, abastecimento, frete, pagamento de frete, custo da OS). Esses módulos não geram lançamento, parcela nem rateio, então o centavo do Financeiro não é tocado. Nunca usar essa constante no Financeiro.
